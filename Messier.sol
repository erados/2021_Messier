...
contract SpaceCampingClub is KIP17Full("Space Camping Club", "MESSIER"), KIP17Mintable, KIP17Burnable, KIP17Pausable {

    string baseURI = "";
    string metadataFormat = ".json";
    string public hash = "";
    string public ifps = "";
    string public contractURL = "";
    uint256 klay = 1000000000000000000;
    uint256 maxNum = 110;
    uint256[] costs = [20 * klay, 40 * klay, 60 * klay, 100 * klay, 200 * klay];
    uint256[] public numberMinted = [0, 0, 0, 0, 0];
    ...
    function setCostByIndex(uint256 _index, uint256 _cost) public onlyOwner {
        costs[_index] = _cost;
    }
    
    function setHash(string memory _hash) public onlyOwner {
        require(bytes(hash).length == 0, "Can be called only once");
        hash = _hash;
    }
    
    function setIFPS(string memory _ifps) public onlyOwner {
        require(bytes(ifps).length == 0, "Can be called only once");
        ifps = _ifps;
    }
    ...
    function mint(uint256 _index) public payable returns (bool) {
        require(numberMinted[_index] < maxNum);
        require(costs[_index] <= msg.value );
        _mint(msg.sender, _index * maxNum + (++numberMinted[_index]));
        return true;
    }
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, idstr, metadataFormat)) : "";
    }
